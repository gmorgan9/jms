...
// UPDATE FULL APPLICATION
	if (isset($_POST['update-full'])) {
		...
		if (mysqli_num_rows($result) > 0) {
			...
			$update = "UPDATE applications SET job_title = NULLIF('$job_title',''), company = NULLIF('$company',''), location = NULLIF('$location',''), job_desc = NULLIF('$job_desc',''), pay = NULLIF('$pay',''), bonus_pay = NULLIF('$bonus_pay',''), status = NULLIF('$status',''), watchlist = '$watchlist', app_link = NULLIF('$app_link',''), job_type = NULLIF('$job_type',''), contact_name = NULLIF('$contact_name',''), contact_email = NULLIF('$contact_email',''), contact_phone = NULLIF('$contact_phone',''), interview_set = '$interview_set', start_date = NULLIF('$start_date',''), resume_used = NULLIF('$resume_used',''), notes = NULLIF('$notes','') WHERE appid = '$id';";
